...
| `scroll-delay` | attribute, non-negative milliseconds | Delays only write-on playback's automatic follow when the pen moves to an off-screen page: `scroll-delay="1000"` keeps the current page visible for one second before following. It does not delay page buttons, `goToPage`, find-result jumps, or manual scrolling. Exact non-negative integer milliseconds only; absent/malformed values mean no delay and values cap at 60,000. Live - read for each new playback page, with no rebuild. A newer playback page, pause/finish/replay/static exit/rerender/disconnect cancels a pending follow, so it cannot later scroll to a stale page. |
...
and write-on renderers without reparsing the note. `dark` and `bare` are also live CSS-only changes; `scroll-behavior` is read on the next component-driven scroll and `scroll-delay` on the next playback page, both without rebuilding.
