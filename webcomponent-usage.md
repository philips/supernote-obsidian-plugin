...
| `scroll-behavior` | attribute, `instant` | E-ink-friendly opt-out from this component's own smooth programmatic scrolling: `scroll-behavior="instant"` makes page jumps (`goToPage` and the toolbar), find-result jumps, and write-on playback's automatic page following jump immediately instead. It does not change manual scrolling. Only exactly `"instant"` opts out; absent or any other value keeps the default smooth behavior. Live - read at the next component-driven scroll, with no rebuild. |
| `scroll-delay` | attribute, non-negative milliseconds | A real-time pause between write-on pages: `scroll-delay="1000"` lets the completed current page remain visible for one second, then moves to the next page while its SVG is still blank and only then resumes its stroke playback. It does not delay page buttons, `goToPage`, find-result jumps, or manual scrolling. Combine with `scroll-behavior="instant"` on e-ink devices for an immediate page switch after the hold. Exact non-negative integer milliseconds only; absent/malformed values mean no pause and values cap at 60,000. Live - a changed value applies at the next page boundary, with no rebuild. Pause/replay/static exit/rerender/disconnect cancels an in-progress hold. |
...
and write-on renderers without reparsing the note. `dark` and `bare` are also live CSS-only changes; `scroll-behavior` is read on the next component-driven scroll and `scroll-delay` at the next write-on page boundary, both without rebuilding.
